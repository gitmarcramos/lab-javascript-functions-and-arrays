...
function findLongestWord(toCheck) {
  if (toCheck.length === 0) {
    return null;
  }
...
function averageWordLength(array) {
  ...
  let arrJoined = array.join('');
  return arrJoined.length / array.length;
}
...
function avg(mixedArr) {
  ...
  return Number((sum / mixedArr.length).toFixed(2));
}
avg(mixedArr2);
